...
public interface ClientRepository extends JpaRepository<ClientEntity, UUID> {
    ...
    @Query("SELECT c FROM ClientEntity c WHERE c.id <> :systemClientId AND c.clientType.name = :clientTypeName")
    Page<ClientEntity> findAllByClientTypeNameExcludingSystem(
            @Param("clientTypeName") String clientTypeName,
            @Param("systemClientId") UUID systemClientId,
            Pageable pageable);

    @Query("SELECT c FROM ClientEntity c WHERE c.id <> :systemClientId AND c.clientType.name = :clientTypeName AND (" +
           "LOWER(c.name) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(c.email) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(c.phone) LIKE LOWER(CONCAT('%', :search, '%')))")
    Page<ClientEntity> searchByNameOrEmailOrPhoneAndClientTypeExcludingSystem(
            @Param("search") String search,
            @Param("clientTypeName") String clientTypeName,
            @Param("systemClientId") UUID systemClientId,
            Pageable pageable);
